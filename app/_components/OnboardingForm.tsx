'use client'

import { useActionState, useState } from 'react'
import { completeOnboarding } from '@/actions/complete-onboarding'
import AddressAutocomplete from './AddressAutocomplete'

const ACTIVITY_LABELS: { key: string; label: string; sublabel: string }[] = [
  {
    key: 'professional_low',
    label: 'Work meeting or class',
    sublabel: 'On average, how long do you take to get ready for a typical work meeting or class?',
  },
  {
    key: 'professional_high',
    label: 'Interview, exam, or networking',
    sublabel:
      'How long do you typically spend preparing for important events like interviews, exams, or networking?',
  },
  {
    key: 'social',
    label: 'Social plans',
    sublabel:
      'How long do you usually take to get ready for social plans (e.g., hanging out, dinner, parties)?',
  },
  {
    key: 'fitness',
    label: 'Workout or physical activity',
    sublabel: 'How long does it take you to get ready for workouts or physical activities?',
  },
  {
    key: 'errands',
    label: 'Errands or appointments',
    sublabel:
      'How long does it usually take you to get ready for errands or appointments (e.g., groceries, therapy, quick tasks)?',
  },
  {
    key: 'special_event',
    label: 'Special events',
    sublabel:
      'How long do you typically spend getting ready for special events (e.g., weddings, conferences, concerts, formal events)?',
  },
]

const inputClass =
  'rounded-lg border border-slate-300 dark:border-zinc-600 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-(--gcal-blue) dark:placeholder-zinc-500'

export default function OnboardingForm() {
  const [state, action, pending] = useActionState(completeOnboarding, null)
  const [step, setStep] = useState(1)
  const [departure, setDeparture] = useState('')
  const [reminderMode, setReminderMode] = useState<'fixed' | 'ai'>('fixed')

  return (
    <form action={action} className="space-y-8">
      {state?.error && (
        <div className="rounded-lg bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 px-4 py-3 text-sm text-red-700 dark:text-red-400">
          {state.error}
        </div>
      )}

      {/* Step 1: Departure */}
      <div className="bg-white dark:bg-zinc-800 rounded-2xl border border-slate-200 dark:border-zinc-700 p-6 space-y-4">
        <h2 className="font-semibold text-lg text-zinc-900 dark:text-zinc-100">1. Where do you usually leave from?</h2>
        <AddressAutocomplete
          name="default_departure"
          value={departure}
          onChange={setDeparture}
          placeholder="e.g. 123 Main St, New York, NY"
          required
          className={`w-full ${inputClass}`}
        />
      </div>

      {/* Step 2: Travel mode */}
      <div className="bg-white dark:bg-zinc-800 rounded-2xl border border-slate-200 dark:border-zinc-700 p-6 space-y-4">
        <h2 className="font-semibold text-lg text-zinc-900 dark:text-zinc-100">2. How do you usually get around?</h2>
        <div className="flex gap-4">
          {(['driving', 'transit', 'walking'] as const).map((mode) => (
            <label key={mode} className="flex items-center gap-2 cursor-pointer">
              <input
                type="radio"
                name="default_travel_mode"
                value={mode}
                defaultChecked={mode === 'driving'}
                className="accent-(--gcal-blue)"
              />
              <span className="text-sm capitalize text-zinc-700 dark:text-zinc-200">{mode}</span>
            </label>
          ))}
        </div>
      </div>

      {/* Step 3: Buffer */}
      <div className="bg-white dark:bg-zinc-800 rounded-2xl border border-slate-200 dark:border-zinc-700 p-6 space-y-4">
        <h2 className="font-semibold text-lg text-zinc-900 dark:text-zinc-100">3. How many minutes early do you like to arrive?</h2>
        <div className="flex items-center gap-3">
          <input
            type="number"
            name="default_buffer_minutes"
            defaultValue={10}
            min={0}
            max={120}
            className={`w-24 ${inputClass}`}
          />
          <span className="text-sm text-zinc-500 dark:text-zinc-300">minutes</span>
        </div>
      </div>

      {/* Step 4: Reminder mode */}
      <div className="bg-white dark:bg-zinc-800 rounded-2xl border border-slate-200 dark:border-zinc-700 p-6 space-y-4">
        <h2 className="font-semibold text-lg text-zinc-900 dark:text-zinc-100">4. How should we remind you to leave?</h2>
        <div className="space-y-3">
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              type="radio"
              name="reminder_mode"
              value="fixed"
              checked={reminderMode === 'fixed'}
              onChange={() => setReminderMode('fixed')}
              className="mt-0.5 accent-(--gcal-blue)"
            />
            <div>
              <div className="text-sm font-medium text-zinc-900 dark:text-zinc-100">Fixed reminder</div>
              <div className="text-xs text-zinc-500 dark:text-zinc-300">Always remind me X minutes before I need to leave</div>
            </div>
          </label>

          {reminderMode === 'fixed' && (
            <div className="ml-6 flex items-center gap-2">
              <input
                type="number"
                name="fixed_reminder_minutes"
                defaultValue={15}
                min={1}
                max={120}
                className={`w-20 ${inputClass}`}
              />
              <span className="text-sm text-zinc-500 dark:text-zinc-300">minutes before leaving</span>
            </div>
          )}

          <label className="flex items-start gap-3 cursor-pointer">
            <input
              type="radio"
              name="reminder_mode"
              value="ai"
              checked={reminderMode === 'ai'}
              onChange={() => setReminderMode('ai')}
              className="mt-0.5 accent-(--gcal-blue)"
            />
            <div>
              <div className="text-sm font-medium text-zinc-900 dark:text-zinc-100">AI / Predictive</div>
              <div className="text-xs text-zinc-500 dark:text-zinc-300">
                AI estimates how long you need to prepare based on the event type
              </div>
            </div>
          </label>
        </div>

        {reminderMode === 'fixed' && (
          <input type="hidden" name="fixed_reminder_minutes" value="15" />
        )}
      </div>

      {/* AI questionnaire */}
      {reminderMode === 'ai' && (
        <div className="bg-white dark:bg-zinc-800 rounded-2xl border border-slate-200 dark:border-zinc-700 p-6 space-y-5">
          <h2 className="font-semibold text-lg text-zinc-900 dark:text-zinc-100">
            How long does it take you to get ready for each activity?
          </h2>
          <div className="grid gap-5 sm:grid-cols-2">
            {ACTIVITY_LABELS.map(({ key, label, sublabel }) => (
              <div key={key} className="space-y-2">
                <div>
                  <p className="text-sm font-medium text-zinc-700 dark:text-zinc-200">{label}</p>
                  <p className="text-xs text-zinc-400 dark:text-zinc-400">{sublabel}</p>
                </div>
                <div className="flex items-center gap-1.5">
                  <input
                    id={`ans_${key}`}
                    type="number"
                    name={`ans_${key}`}
                    defaultValue={15}
                    min={0}
                    max={120}
                    className={`w-16 text-right ${inputClass}`}
                  />
                  <span className="text-xs text-zinc-500 dark:text-zinc-300">min</span>
                </div>
              </div>
            ))}
          </div>
        </div>
      )}

      <button
        type="submit"
        disabled={pending}
        className="w-full rounded-xl bg-(--gcal-blue) py-3 text-sm font-semibold text-white hover:opacity-90 transition-opacity disabled:opacity-50"
      >
        {pending ? 'Setting up…' : 'Get started'}
      </button>
    </form>
  )
}
